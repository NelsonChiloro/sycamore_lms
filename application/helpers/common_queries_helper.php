...
function get_loan_outstanding_balance($loan_id) {
    ...
    // Outstanding balance rule: installments due up to today minus paid up to today.
    $row = $CI->db
        ->select('COALESCE(SUM(amount), 0) AS total_expected, COALESCE(SUM(paid_amount), 0) AS total_paid', false)
        ->from('payement_schedules')
        ->where('loan_id', $loan_id)
    ...
    $expected = isset($row->total_expected) ? (float) $row->total_expected : 0.0;
