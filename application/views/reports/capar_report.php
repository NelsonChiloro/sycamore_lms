...
<div class="main-content">
	<div class="page-header">
		<h2 class="header-title">CAPAR (Capital At Risk) Report</h2>
		<div class="header-sub-title">
			<nav class="breadcrumb breadcrumb-dash">
				<a href="<?php echo base_url('Admin')?>" class="breadcrumb-item"><i class="anticon anticon-home m-r-5"></i>Home</a>
				<a class="breadcrumb-item" href="#">-</a>
				<span class="breadcrumb-item active">Capital At Risk Report</span>
			</nav>
		</div>
	</div>
	<div class="card">
		<div class="card-body" style="border: thick #153505 solid;border-radius: 14px;">
			<form action="<?php echo base_url('reports/caparfilter') ?>" method="get">
				<fieldset>
				...
				<div id="employee_table">
					<table id="resulta"  class="table" cellspacing="1">
						...
						<tr>
							...
							<th>Total Principal Balance Outstanding</th>
						...
						<?php if($summary) : ?>
							<?php foreach ($summary as $row) :?>
								<?php
										...
										// Calculate principal balance from unpaid payment schedules
										$principal_balance = 0;
										$oldest_arrears_date = null;
										$unpaid_schedules = get_unpaid_principal_balance($row->loan_id);
										if($unpaid_schedules && isset($unpaid_schedules->total_principal)){
											$principal_balance = $unpaid_schedules->total_principal;
										}
										
										// Get oldest unpaid schedule date for aging calculation
										$oldest_unpaid = get_oldest_unpaid_schedule($row->loan_id);
										if($oldest_unpaid && isset($oldest_unpaid->payment_schedule)){
											$oldest_arrears_date = $oldest_unpaid->payment_schedule;
										}
										?>
										...
										<tr>
											...
											<td><?php
                                                echo number_format($principal_balance, 2, '.', ',') ;
												$tprincipal += $principal_balance;
											?></td>

                                           <td><?php
												if($oldest_arrears_date) {
													$date=date("Y-m-d H:i:s");
													$dateOne = new DateTime($date);
													$dateTwo = new DateTime($oldest_arrears_date);
													$diff = $dateTwo->diff($dateOne)->format("%a");
													
													if($diff > 0){
														$tmorethanone += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=0 && $diff<=7){
														$tzerotoseven += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=8 && $diff<=30){
														$morethanseven += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=31 && $diff<=60){
														$morethanthirty += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=61 && $diff<=90){
														$morethansixty += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=91 && $diff<=120){
														$morethanninety += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=121 && $diff<=180){
														$morethanonetwenty += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=181 && $diff<=366){
														$morethanoneeighty += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
											...
											<td><?php
												if($oldest_arrears_date) {
													...
													if($diff>=367){
														$morethanthreesixty += $principal_balance;
														echo number_format($principal_balance, 2);
													} else {
														echo '0';
													}
						...
						<tr>
							...
							<td>-</td>
							<td>-</td>
							<td><?php echo "MK" .number_format($tprincipal,2); ?></td>
							<td><?php echo "MK" .number_format($tmorethanone,2); ?></td>
...
if(empty($this->session->userdata('officerid') )AND empty($this->session->userdata('productid') ) ) {
    ...
    $p1 = $totalunpaid->total_not_paid;
						...
						<tr>
							...
							<td></td>
							<td></td>
							<td><?php echo round(($tprincipal/$totalunpaid->total_not_paid)*100,2).'%'; ?></td>
							<td></td>
							<td></td>
							<td></td>
							<td></td>
						...
						<tr>
							...
							<td></td>
							<td></td>
							<td><?php echo round(($tmorethanone/$p1)*100,2).'%'; ?></td>
							<td></td>
							<td></td>
							<td></td>
							<td></td>
