...
class Group_batch_model extends CI_Model
{
    ...
    public function build_member_edit_snapshots($loan_row, $product_row, $posted)
    {
        ...
        } elseif (!empty($posted['period_type'])) {
            $period_type = trim((string) $posted['period_type']);
        }
    ...
    public function apply_approved_batch_edit($approval_row)
    {
        ...
        try {
            foreach ($payload->members as $member) {
                ...
                $product_id = isset($member->sy_loan_product) ? (int) $member->sy_loan_product : 0;
                if ($product_id <= 0 && $shared && isset($shared->sy_loan_product)) {
                    $product_id = (int) $shared->sy_loan_product;
                }
                if ($product_id > 0) {
                    $product = $this->db->select('frequency')->from('loan_products')->where('loan_product_id', $product_id)->get()->row();
                    if ($product && !empty($product->frequency)) {
                        $period_type = trim((string) $product->frequency);
                    }
                }
